...
#### `arqc-parameters-manager` → `Bmg.Parameters.Manager`
...
- **`[BmgDynatraceTrace]`** — atributo obrigatório em todo método público de `Service` e `AppService` para rastreamento distribuído
...
- `AddWather[Contexto]ApiModule()` — registra Controllers, AppServices e interfaces
- `AddWather[Contexto]ApplicationModule()` — registra Services do Domain
...
- [ ] Atributo `[BmgDynatraceTrace]` no AppService e no Application Service
